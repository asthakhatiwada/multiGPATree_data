...
options(digits = 2)
t1 <- fread('setting1_simstudy_aim2_tree_case_09152023.txt')
# t1 <- fread('setting2_simstudy_aim2_tree_case_09152023.txt')
...
save(sim_results, file = "setting2_multiGPATree_simulation_results_data_09172023.RData")


datfile <- sim_results
# 1.1 A1-A6 simultaneously selected ####
d <- datfile[,c('P2_alpha_true', "percent_overlap_ann", "multiGPATree_fitSelectVar")]
d$all6ann_sel <- ifelse(d$multiGPATree_fitSelectVar == 'A1, A2, A3, A4, A5, A6', 1, 0)
...
dnew1$wrong_num_ann_select[i] <- length(which(ann_select > 6))
dnew1$FDR_ann[i] <- dnew1$wrong_num_ann_select[i]/dnew1$total_num_ann_select[i] 
dnew1$TDR_ann[i] <- (dnew1$total_num_ann_select[i] - dnew1$wrong_num_ann_select[i])/6 
}
names(dnew1)
...
mutate(criteria = recode(criteria, 
                         mean_FDR = "Mean % of noise annotations (A7-A25) \namong selected annotations", 
                         prop_all6ann = "% of simulated data for which A1-A6 \nare simultaneously selected ", 
                         mean_TDR = "Mean % of true annotations (A1-A6) \nthat are selected"))
...
ggsave(file ='setting1_simulationResult_09172023.png',
       # file ='setting2_simulationResult_09172023.png',
       saveplot,
       width = 10.5, height = 8)
